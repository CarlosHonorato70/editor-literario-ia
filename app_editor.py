...
def verificar_estilo_gramatica_ia(texto: str) -> Dict:
    ...
    try:
        ...
        return {"status": "Erro na resposta da IA", "sugestoes": [{"tipo": "erro", "descricao": "Resposta da IA não é um JSON válido."}], "texto_corrigido": texto}
...
with revisao_tab:
    ...
    if st.button("✨ Verificar Estilo e Gramática com IA"):
        if not texto_para_verificar.strip():
            ...
            if "[ERRO DE CONEXÃO DA API]" in analise.get("status", "") or "Erro na resposta da IA" in analise.get("status", ""):
...
with referencias_tab:
    ...
    if st.button("📚 Formatar Referências com IA"):
        if not texto_com_cit.strip():
            ...
            if refs and "[ERRO DE CONEXÃO DA API]" in refs[0].get("descricao", ""):
...
with colaboracao_tab:
    ...
    if st.button("🤝 Sugerir Revisões Colaborativas com IA"):
        if not texto_original_colab.strip() or not texto_editado_colab.strip():
            ...
            if sugestoes_colab and "[ERRO DE CONEXÃO DA API]" in sugestoes_colab[0].get("descricao", ""):
...
with slides_tab:
    ...
    if st.button("📊 Gerar Slides com IA"):
        if not texto_base_slides.strip():
            ...
            if slides_gerados and "[ERRO DE CONEXÃO DA API]" in slides_gerados[0].get("conteudo", ""):
